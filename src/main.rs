            ...
            if position == 0. {
                if prev_price > prev_bb_band.up || prev_price < prev_bb_band.down {
                    let side = if prev_price > prev_bb_band.up {
                        trade_side::SELL as f64
                    } else {
                        trade_side::BUY as f64
                    };
                    position = side * size;
                    entry_price = curr_price;
                    take_profit_price = entry_price * (1. + take_profit_percentage * side);
                    stop_loss_price = entry_price * (1. - stop_loss_percentage * side);
                    let fee = calculate_fee(fee_rate, entry_price, size, leverage);
                    total_fee += fee;
                    usd_balance -= fee;
                }
                ...
                if mid_price >= take_profit_price || mid_price <= stop_loss_price {
                    ...
                    let profit = (exit_price - entry_price) * position * leverage;
                    let fee = calculate_fee(fee_rate, curr_price, size, leverage);
                    usd_balance -= fee;
                    usd_balance += profit;
                    total_fee += fee;
                    total_profit += profit;
